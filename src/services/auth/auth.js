...
import {getAdminAccessStatus} from "../admin/accessReqService";
...
export const checkAuth = async () => {
  try {
    const auth_response = await axiosInstance.get("auth/check.php?portal=admin");

    if (auth_response?.status === 200) {
      try {
        const access_response = await getAdminAccessStatus();
